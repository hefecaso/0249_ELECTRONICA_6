...
GPIO.setup(servo, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) #configurar el pin 13 como entrada e internamente una resistencia pull down
...
pi.set_servo_pulsewidth(servo,2390) #girar el servo hasta 0 grados como inicio
...
try:
        ...
        GPIO.wait_for_edge(servo, GPIO.RISING, timeout=5000) #configurar el pin 13 una interrupcion de tipo espera, con un maximo tiempo de 5 segundos
        while True:
                ...
                if grados < 0 or grados >180:
                    ...
                    pi.set_servo_pulsewidth(servo,510)
                elif grados < 10:
                        Nms = grados*4+515
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 10:
                        pi.set_servo_pulsewidth(servo,555)
                elif grados < 20:
                        Nms = (grados-10)*8+555
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 20:
                        pi.set_servo_pulsewidth(servo,635)
                elif grados < 30:
                        Nms = (grados-20)*9+635
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 30:
                        pi.set_servo_pulsewidth(servo,725)
                elif grados < 40:
                        Nms = (grados-30)*9.5+725
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 40:
                        pi.set_servo_pulsewidth(servo,820)
                elif grados < 45:
                        Nms = (grados-40)*1+820
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 45:
                        pi.set_servo_pulsewidth(servo,825)
                elif grados < 50:
                        Nms = (grados-45)*25+825
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 50:
                        pi.set_servo_pulsewidth(servo,950)
                elif grados < 60:
                        Nms = (grados-40)*10+950
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 60:
                        pi.set_servo_pulsewidth(servo,1050)
                elif grados < 70:
                        Nms = (grados-60)*8+1050
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 70:
                        pi.set_servo_pulsewidth(servo,1130)
                elif grados < 80:
                        ...
                        pi.set_servo_pulsewidth(servo,1425)
                elif grados < 110:
                        Nms = (grados-100)*16.5+1425
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 110:
                        pi.set_servo_pulsewidth(servo,1590)
                elif grados == 122:
                        ...
                        pi.set_servo_pulsewidth(servo,1825)
                elif grados < 140:
                        Nms = (grados-130)*11.5+1825
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 140:
                        pi.set_servo_pulsewidth(servo,1940)
                elif grados < 150:
                        Nms = (grados-140)*11+1940
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 150:
                        pi.set_servo_pulsewidth(servo,2050)
                elif grados < 160:
                        Nms = (grados-150)*12+2050
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 160:
                        pi.set_servo_pulsewidth(servo,2170)
                elif grados < 170:
                        Nms = (grados-160)*11+2170
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 170:
                        pi.set_servo_pulsewidth(servo,2280)
                elif grados < 180:
                        Nms = (grados-170)*10+2280
                        pi.set_servo_pulsewidth(servo,Nms)
                elif grados == 180:
                        pi.set_servo_pulsewidth(servo,2390)
                time.sleep(1)
